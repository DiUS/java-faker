...
public class Number {
  ...
  public int numberBetween(int min, int max) {
     ...
     * if the min is greater than max then it would swap the value
  ...
  public long numberBetween(long min, long max) {
     ...
     * @param strict         whether or not the generated value should have exactly
     *                       <code>numberOfDigits</code>
     */
  ...
  private BigDecimal decimalBetween(long min, long max) {
    ...
    if(chunkStart + adj >= trueMax){
        return new BigDecimal(trueMax - 1);
    }else{
