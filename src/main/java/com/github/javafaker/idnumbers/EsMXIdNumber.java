...
public class EsMXIdNumber {
    ...
    /**
     * Randomly gets a sex.
     * @param faker A specific instance of Faker.
     * @return A valid sex.
     */
    private String getSex(Faker faker){
        return new String[]{"H", "M"}[faker.random().nextInt(2)];
    }
    ...
    public String getWrong(){
        String CURP = otherLetter(first4Letter()+birthDay.substring(2,8)+sex+state+middle3Consonant());
        Random random = new Random();
        int a = random.nextInt(4);
        int b = random.nextInt(6) + 4;
        int c = random.nextInt(6) + 10;

        CURP = CURP.substring(0,a)+"X"+CURP.substring(a+CURP.length());
        CURP = CURP.substring(0,b)+"4"+CURP.substring(b+CURP.length());
        CURP = CURP.substring(0,c)+"N"+CURP.substring(c+CURP.length());
